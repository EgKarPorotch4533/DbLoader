	...
	public void run() {
		...
		try {
			...
			while(!partitionQueue.isEmpty()) {
				...
				final String query = new SelectFromTable(tableName, schemaName, partition).getQuery();
				System.out.printf("[%d] starting read new partition %s from table %s.%s [%s]%n",
						Thread.currentThread().getId(), partition, schemaName, tableName, query);
				try {
